const main = async () => {
  ...
  new_tab_button.innerHTML = (await chrome.storage.sync.get("new_tab")).new_tab;
  new_tab_button.addEventListener("click", async (e) => {
    const state = (await chrome.storage.sync.get("new_tab")).new_tab as boolean;
    await chrome.storage.sync.set({ new_tab: !state });
    new_tab_button.innerHTML = (
      await chrome.storage.sync.get("new_tab")
    ).new_tab;
  });

  const force_download_button = document.createElement("button");
  force_download_button.innerHTML = (
    await chrome.storage.sync.get("force_download")
  ).force_download;
  force_download_button.addEventListener("click", async (e) => {
    const state = (await chrome.storage.sync.get("force_download"))
      .force_download as boolean;
    await chrome.storage.sync.set({ force_download: !state });
    force_download_button.innerHTML = (
      await chrome.storage.sync.get("force_download")
    ).force_download;
  });
